...
class DutchieBannerCarousel {
    ...
    public function output_styles() {
        echo '<style>
.dutchie-carousel{position:relative;overflow:hidden!important;max-width:100%;width:100%;cursor:grab;touch-action:pan-y pinch-zoom;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none;box-sizing:border-box}
.dutchie-carousel.dragging{cursor:grabbing}
.dutchie-track{display:flex!important;will-change:transform}
...
.dutchie-zoom-lens{position:fixed;width:150px;height:150px;border-radius:50%;border:3px solid rgba(255,255,255,.9);box-shadow:0 4px 20px rgba(0,0,0,.4);pointer-events:none;display:none;z-index:10000;overflow:hidden;background-repeat:no-repeat;background-color:#000}
...
@media(max-width:768px){.dutchie-slide{aspect-ratio:2/1}.dutchie-slide img{object-fit:cover!important;height:100%!important}.dutchie-counter{display:block}.dutchie-prev,.dutchie-next{padding:10px 8px;font-size:16px;background:rgba(0,0,0,.3)}.dutchie-dots{bottom:6px}.dutchie-dot{width:8px;height:8px;margin:0 4px;background:rgba(255,255,255,.5)}.dutchie-dot.active{background:#fff;transform:scale(1.3)}}
        ...
        if (!empty($opts['custom_css'])) {
            ...
            $css = str_replace(array('</style', '<style', '<script', '</script'), '', $css);
            echo '<style id="dutchie-custom-css">' . $css . '</style>';
        }
    }
}
